 ...
 * @async
 */
Server.hasJoined = function (username, serverid, sharedsecret, serverkey, ip, cb) {
  const host = this.host || defaultHost
  const hash = utils.mcHexDigest(crypto.createHash('sha1').update(serverid).update(sharedsecret).update(serverkey).digest())
  console.log( `${host}/session/minecraft/hasJoined?username=` + require('querystring').escape(`${username}`) + `&serverId=` +  require('querystring').escape(`${hash}`) + `&ip=` +  require('querystring').escape(`${ip}`))
  utils.phin({
    url: `${host}/session/minecraft/hasJoined?username=` + require('querystring').escape(`${username}`) + `&serverId=` +  require('querystring').escape(`${hash}`) + `&ip=` +  require('querystring').escape(`${ip}`),
    core: {
      agent: this.agent
    }
  }, function (err, resp) {
